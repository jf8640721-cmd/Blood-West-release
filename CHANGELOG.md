...
在 v2.0.3「暗色画框」基础上进一步升级为沉浸式暗黑奇幻风格——皮革纹理背景、中式金色角花装饰、华丽多层边框。

**改动内容：**
- `admin/assets/` — 新增 6 个 SVG 资源文件（皮革纹理、面板装饰框、弹窗装饰框、如意角花、金色分隔线、暗纹底纹）
- `admin/style.css` — :root 变量微调（古铜金更暖更亮）、body 叠加皮革纹理、新增 `.ornate-panel` / `.ornate-modal` 装饰类、工具栏/面板/圆桌/弹窗/按钮全面增强金色装饰
- `admin/index.html` — 面板添加 `.ornate-panel` 类、弹窗添加 `.ornate-modal` 类、版本号 v2.0.3 → v2.1.0
- `miniprogram/pages/scan/scan.wxml` — 版本号 v2.0.3 → v2.1.0

## v2.0.3 (2026-06-09)

### 🎨 UI 风格重设计：暗色画框

基于74张暗黑西游卡牌图片的深度分析（色彩丰富度、饱和度分布、色相统计），将主持后台 UI 从冷紫暗色调全面重构为「暗色画框」风格——深色、安静、退后，让角色和内容的色彩自己发光。

**设计来源：** UI/UX Pro Max 技能综合搜索（Modern Dark Cinema + Theater spotlight gold + Calculator warm dark）

**改动内容：**
- `admin/style.css` — CSS 设计系统变量全面替换（5级暖暗背景 + 古铜金强调 + 暖灰白文字 + 低对比边框）
- 全局基础：噪点纹理降透明度、工具栏/Logo/按钮色调调暖
- 组件样式：消息气泡、弹窗、队列面板、邪恶群聊配色微调
- 保留邪恶阵营/夜间队列紫色主题以维持功能区分
