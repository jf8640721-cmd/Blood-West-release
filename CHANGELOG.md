...
28 个拥有多个子技能的角色，玩家现在可以选择发动哪一个，主持人也能识别。

**解析**：`admin/data/roles.js` + `miniprogram/utils/roles.js` 新增 `parseSubSkills(abilityStr)` 工具函数，按 `；` 分号自动拆分 `name：description` 对
**小程序端**：技能栏角色名下方新增子技能按钮组 `[归元] [化墟]`，点击高亮选中 → 提交时 `action_data` 写入 `skillName` + `skillIndex`，消息格式变为 `🎯【菩提祖师 · 归元】发动技能`
**Web 端**：队列/处理弹窗/历史记录均显示"角色名 · 子技能名"，单技能角色行为完全不变
...
- **问题**：菩提祖师仅有 `nightOrder: 14`，缺少 `dayOrder`，导致白天技能栏不显示
- `admin/data/roles.js`：菩提祖师添加 `dayOrder: 14`
- `admin/js/queue.js`：`DAY_ACTIONS` 添加菩提祖师条目（`timing: '全局一次'`）
...
- `room.js`：`editPlayerNickname` 升级为号码+名牌同步编辑
  - `prompt` 格式改为 `"号码 名牌"`（如 `3 Lester`）
  - 自动解析输入：第一段为数字 → 新号码，否则全当名牌
  - 号码冲突检测（同房间 `state.players` 内存比对）
  - 号码和名牌各自判断是否变化，无变化则跳过 PATCH
- `table.js`：左侧玩家列表名牌前显示号码（金色数字）
- `style.css`：新增 `.player-num` 样式
- 主持人始终可修改，不受阶段限制
...
- `admin/app.js`：`debugAddTestPlayers` 模拟加入的玩家昵称从角色名（孙悟空、猪八戒…）改为号码格式（1号、2号…），避免测试时混淆
...
- **修复**：沙僧/唐太宗/东海龙王等所有白天技能角色，进入白天时小程序端不显示技能发动界面
- **根因**：`evaluateSkillBar()` 仅判断 `isNightPhase && hasNightOrder`，完全未处理白天技能
- `chat.js`：新增白天判断分支 `isDayPhase && hasDayOrder`，`dayOrder < 20` 显示主动技能栏（与夜间表现一致）
...
- 移除玩家私聊输入框下方的快捷回复按钮（✅ 已收到 / ❓ 请说明 / 🔒 已处理 / 🌪️ 选元素）
- 清理相关 JS（`sendQuickReply`）、CSS（`.btn-quick-reply`）和事件绑定
...
- 版型配置弹窗新增搜索框：输入角色名快速过滤当前版型中的角色
- 搜索结果自动显示"可添加角色"，点击即可加入版型（新增角色闪烁高亮）
- 角色交换从原生 `<select>` 升级为可搜索输入框（`<input>` + `<datalist>`）
- 支持模糊匹配：输入拼音或部分名称即可定位
